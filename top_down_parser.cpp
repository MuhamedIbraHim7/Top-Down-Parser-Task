...
int check_if_accept(string input, string rules[], int no_rules, char start_non_term){
	//cout << input << endl;
	stack<char> s;
	s.push('$'); // $ end marker
...
int main(){
    ...
    cout << "\nEnter input ending with $:";
   	...
   	cout << (check_if_accept(input, rules, no_rules, start_non_term)?"Accepted":"Not Accepted") << endl;
    return 0;
}
